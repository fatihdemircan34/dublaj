...
def quality_metrics(json_path):
    try:
        data = load_json(json_path)
    except Exception:
        return {"ok": False, "amb_seg": 1e9, "empty_words": 1e9}
    ...
    if found is None:
        ...
        for p in sorted(out_dir.glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True):
            try:
                ...
                continue
    if found is None:
        raise SystemExit(f"WhisperX JSON bulunamadı. out_dir='{out_dir}' içinde aday yok.")
...
def main():
    ap = argparse.ArgumentParser(description="WhisperX -> Post-process (reassign/overlap/auto/both) tek komut")

    # Zorunlu
    ap.add_argument("--audio", required=True, help="Giriş ses yolu (wav/flac/ogg)")
    ...
    ap.add_argument("--postproc", default="auto", choices=["reassign", "overlap", "both", "auto"],
                    help="Post-process seçimi")
    ap.add_argument("--out_dir", default="out_whisperx", help="WhisperX ve ara çıktılar için klasör")

    # Betiklerin konumu
    ap.add_argument("--scripts_dir", default=None,
                    help="overlap_diarize_and_fuse.py ve reassign_speakers.py'nin bulunduğu klasör")
    ...
    # 1b) WhisperX JSON’u bul ve sabitle
